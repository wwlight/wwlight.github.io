...
1. 为账户生成独立的 `SSH` 密钥
    ...
    <Tabs syncKey="pkg">
        <TabItem label="ed25519">
            ```sh frame="none" del="personal@email.com" ins="id_ed25519_github"
        ...
        <TabItem label="rsa">
            ```sh frame="none" del="personal@email.com" ins="id_rsa_github"
...
- 在 `powershell` 目标路径后追加 `-NoLogo` 或者 `-nologo`
- 在 `cmd` 目标路径后追加 `-NoLogo /k` 或者 `-nologo /k`
