...
4.  配置 `COMPOSE_FILE`
    ...
    ```sh
    $ mkdir -p ~/.docker && : >> ~/.docker/compose.yml
...
Legacy 顶层命令，等价于下方 [`docker container`](https://docs.docker.com/reference/cli/docker/container/) / [`docker image`](https://docs.docker.com/reference/cli/docker/image/) 子命令。
