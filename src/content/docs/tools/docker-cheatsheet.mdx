...
:::note[Colima]
[Colima](https://colima.run/) 是 macOS（及 Linux）上的容器运行时，以最小配置即可运行 Docker 和 Containerd，支持 Intel 和 Apple Silicon。适用于替代 Docker Desktop、本地容器开发、多实例管理、Kubernetes 集群（--kubernetes）、Containerd 运行时、GPU 加速 AI 容器（--vm-type krunkit）、Rosetta 2 模拟（--vz-rosetta）等场景。
:::
