...
:::note[Headroom]
[Headroom](https://github.com/headroomlabs-ai/headroom) 本地上下文压缩层：LLM 收到请求前压缩 tool output、日志与对话历史，典型节省 60-95% token（CCR 可逆）。上游接 [OmniRoute](/tools/omniroute-setup/)，下游模型无感。与 [RTK](https://github.com/rtk-ai/rtk) 互补——RTK 裁终端输出，Headroom 裁 API 上下文。详见 [官方文档](https://headroom-docs.vercel.app/docs)。
:::
...
1. 添加 Headroom 服务
    ...
    services:
      headroom:
        ...
        depends_on:
          - omniroute
        healthcheck:
          test: ["CMD", "curl", "--fail", "--silent", "http://127.0.0.1:20129/readyz"]
          interval: 30s
          timeout: 5s
          start_period: 20s
          retries: 3
    ...
    `command` 覆盖镜像默认的 `--port 8787`，使容器内也监听 `20129`（仅设 `HEADROOM_PORT` 无效）。`OPENAI_TARGET_API_URL` 将压缩后的请求转发到同 compose 内的 OmniRoute。
