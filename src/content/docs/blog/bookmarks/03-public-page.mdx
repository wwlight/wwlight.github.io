---
title: 03 · 导航页与客户端渲染
description: entry.astro、JSON 注水与 React 导航组件
sidebar:
  order: 3
---
...
head 内联 `@/theme/scripts/init.inline.js?raw`，在 CSS 前写入 `data-theme` / `data-color-*`，避免 FOUC。`ThemeProvider`（`@/theme/components/color-mode/Provider`）mount 时 `syncSiteThemeFromStorage()`，并订阅 `subscribeSiteThemeStorage`——与 Starlight 顶栏定制器读写同一套 `wwlight:color-mode` 等键（见 [主题系列](/blog/theme/03-color-mode-and-sync/)）。
