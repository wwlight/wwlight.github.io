...
管理端 `/bookmarks/admin/` 需要一层 **访问控制**：防止路人打开就改数据。在纯静态 + 本地 API 的架构下，采用 **前端密码门控 + Bearer Token** 方案。
...
3. 启动 dev（`scripts/dev-bootstrap.mjs`），自动打开 `/bookmarks/admin/`
...
核心逻辑在 `src/bookmarks/admin/lib/admin-auth.ts`：
