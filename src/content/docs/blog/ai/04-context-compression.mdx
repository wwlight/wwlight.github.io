...
| **rtk** | Shell 命令的 **终端 stdout** | 命令执行层（preToolUse hook） |
...
:::note[自定义上游验证]
非 Docker 部署时，确认 `OPENAI_TARGET_API_URL` 已正确设置：`curl -s http://127.0.0.1:8787/v1/models` 应返回上游的模型列表而非 `connection_error`。
:::
...
写入 **`~/.config/opencode/opencode.json`** 的 `mcp` 字段：
...
2. **重启 Agent 客户端**，在对应设置面板确认 `headroom` 连接状态
   ...
   - **OpenCode**：`opencode.json` 中 `enabled: true`，启动后自动加载
...
上行中 proxy 监听 `:8787`，所有请求经压缩后转发到 `http://localhost:20128/v1`。`OPENAI_TARGET_API_URL` 指向自定义 OpenAI 兼容 API 时，无需再设置 `OPENAI_API_KEY`。
...
| Provider Base URL 指向 proxy | 全量自动压缩（OpenCode 配置文件 / Cursor Override OpenAI Base URL / CLI 环境变量） |
...
### 5. Agent 内：`headroom_stats` MCP

在 Agent 对话中调用 `headroom_stats`，或在 MCP 管理面板确认 `headroom` 已连接后直接询问。返回 `compressions`、`tokens_saved`、`recent_events`（最近 10 条压缩/取回记录）。
...
### macOS launchd 服务（非 Docker）

本地安装时注册为 launchd 服务实现开机自启：
...
Plist 文件需配置 `OPENAI_TARGET_API_URL` 环境变量。详见 [Headroom 安装配置](/tools/headroom-cheatsheet/)。
