...
**设计权衡**：`alwaysApply: true` 保证约束必达，但每条增加约 500–2000 token 的固定开销。适合放全局编码准则；不适合放大段 API 文档。
...
2. **单一入口** — alwaysApply Rule 或 MCP 按需读取提供统一项目知识入口（本仓库：Serena `mem:core`）
...
| 项目知识 | `.serena/memories/core.md` + `mem:conventions` | 目录结构、路由、命令、Tailwind v4、组件规范、反模式 |
