...
`PublicBookmarksPage.tsx` / `BookmarksAdmin.tsx`：
...
- 管理端 `AdminHeaderActions` 与公开页 `PublicPageActions` 均挂 `ColorThemePicker`

全站 preference 一致：在文档站改 primary 后打开 `/bookmarks/`，init 或 `storage` 事件应用相同 `data-color-primary`。
