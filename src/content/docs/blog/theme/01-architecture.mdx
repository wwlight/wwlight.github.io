...
Primary 在亮色取 Tailwind `500`、暗色取 `400`；`black` 的 `--primary` 亮 `zinc-950` / 暗 `white`（见 `color-tokens.css`）。Panel 里 black 预览色块见 `customizer-trigger.css` 的 `--theme-primary-swatch-black`。
...
| 书签 / 管理端 | `src/pages/bookmarks/index.astro`、`admin/bookmarks.astro` 内联 `<script>` |
...
- `@/theme/components/ColorThemeSelect.astro` — Starlight 静态触发按钮
- `@/theme/components/ColorThemePicker` — 书签 / 管理端 React 触发器别名
...
- `trigger-classes.ts`、`surface.ts` 为 `starlight` / `bookmarks` 各写一套 Tailwind，未抽共享 design token
