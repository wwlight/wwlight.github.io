...
- `scripts/color-themes.data.mjs` 生成 `neutral-scales.css`、`color-tokens.css` 与选项清单
...
flowchart TB
  ...
  subgraph css["样式"]
    Scales["neutral-scales.css<br/>扩展 neutral @theme"]
    Tokens["color-tokens.css<br/>primary / neutral / radius"]
...
| 书签组件色 | shadcn 语义变量（Tailwind `--color-*` step） | Radix + `@theme inline` 消费 |
...
| 跨表面 UI | `customizer-ui.css` + `surface.ts` | 触发器 chrome 用 `--theme-ui-*`；Panel 选项 hover 分表面 |
...
局部文档见 [`src/theme/README.md`](https://github.com/wwlight/wwlight.github.io/blob/main/src/theme/README.md)。书签 / 文档站见 [书签系列](/blog/bookmarks/) / [Astro 使用](/blog/astro/)。
