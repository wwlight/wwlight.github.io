...
`black` 的 accent 映射 `zinc`；`tailwindPrimaryTokens` 亮 `--color-zinc-950`、暗 `--color-white`。Panel swatch 为 `var(--color-zinc-950)`，与其余 primary 一样引用 Tailwind 色阶。

## Neutral：Tailwind 灰阶 + 扩展色阶

Neutral 共 9 项：Tailwind 内置五档 + 4 个扩展名（`taupe` / `mauve` / `mist` / `olive`）。扩展色由生成器从内置 scale 的 **L 曲线** 推导 oklch，并固定各自 **hue**（写入 `neutral-scales.css` 的 `@theme --color-{name}-*`）；表面语义仍由 `tailwindNeutralSurfaceTokens()` 按 step 映射。Panel swatch 为 `var(--color-{scale}-500)`，九项互不相同。`grayScale()` 把 `--color-gray-*` 重映射到所选 scale，供 Starlight `--sl-color-gray-*` 链使用。
...
| `--background` 等 | neutral 块 | 仅 `@/styles/global.css` 引入的 shadcn 岛 | 整页背景、Card、Popover |
